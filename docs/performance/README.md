...
This benchmark was produced on the historical `codex/exact-performance` branch.
The later integrated codebase supersedes the branch's core caching and indexing
implementation with newer exact execution paths, while retaining the benchmark,
worker memory improvements, and regression intent as provenance. The distinct
source hash in `assessment.json` therefore identifies the historical benchmark
candidate and must not be relabeled as the current integrated source. Benchmark
reproduction remains available through `scripts/benchmark_exact_performance.py`
with explicit source-root and case-file paths.
