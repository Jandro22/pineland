...
The visual annotates the relief-to-yield transition with static headroom while
making clear that realized yield also depends on system adaptation and induced
demand. The yield-to-retention transition highlights support-enabled demand and
constraint migration. Migration is a mechanism, not itself the measure of
success.
...
**Status:** Implemented in `manuscript.md` after source audit.

The table preserves mixed and adverse evidence, especially Mali's pre-existing complementary weaknesses and Colombia's uneven nationalization across related aviation institutions.

## Figure 7. Coordinated development and the rising-tide test

**Status:** Generated as `figures/figure7_stage5_complementarity.{png,pdf}` from completed frozen Stage-5 outputs.

**Source:** Stage-5 analysis outputs generated by
`analyze_stage5_rising_tide.py`.

The final form shows both:
