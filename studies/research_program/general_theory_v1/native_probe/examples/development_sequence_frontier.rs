...
fn schedule_multiplier(schedule: &str, time: f64) -> f64 {
    match schedule {
        "steady5" => 5.0,
        "immediate20" => 20.0,
        "delayed20" => {
...
fn run_case(
    ...
    let base_budget = e.config.political_order.federal_policy_budget;
    ...
    while time <= final_day + 1.0e-9 {
        let applied_multiplier = schedule_multiplier(schedule, prior);
        e.config.political_order.federal_policy_budget = base_budget * applied_multiplier;
        ...
        let s = snapshot(&e);
        rows.push(format!(
            "{seed},{schedule},{applied_multiplier:.6},{time:.6},{anchor_capital:.17},{:.17},{:.17},{:.17},{:.17},{:.17},{:.17},{:.17},{:.17},{:.17},{:.17},{:.17},{:.17}",
            s.capital,
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let out = args
        .get(1)
        .cloned()
        .unwrap_or_else(|| "../development_sequence_frontier_v1.csv".into());
    ...
    let seed_base: u64 = arg(&args, 6, 2026130000u64);
    ...
    let schedules = ["steady5", "immediate20", "delayed20", "ramp20"];
    let tasks = anchors_ok
        .iter()
        .flat_map(|(seed, e)| schedules.iter().map(move |s| (*seed, e, *s)))
        .collect::<Vec<_>>();
    let results: Vec<Result<Vec<String>, String>> = pool.install(|| {
        tasks
            .par_iter()
            .map(|(seed, e, s)| run_case(*seed, e, s, final_day))
            .collect()
    });
    let mut w = BufWriter::new(File::create(&out)?);
    writeln!(w,"seed,schedule,applied_budget_multiplier,time,anchor_government_capital,government_capital,mean_government_legitimacy,mean_state_legitimacy,mean_political_access,mean_local_institution_capacity,population_weighted_government_control,interval_inflow,interval_outflow,interval_political_outflow,interval_governance_outflow,interval_state_regeneration_outflow,interval_other_outflow")?;
    for r in results {
    ...
    w.flush()?;
    println!(
        "wrote {out} seeds={seeds} schedules=4 rows={}",
        seeds * 4 * 13
    );
    Ok(())
}
