...
GROUPS = [
    ...
    },
    {
        ...
        "role": "Implemented; integrated Partner-Force Stage 4 completed",
        "py": [
            "tests/test_foreign_affairs.py",
            "tests/test_arc_hpc_campaign.py",
            "tests/test_stage4_compact_evidence.py",
        ],
        "rs": [
        ...
        "docs": [
            ...
            "studies/research_program/general_theory_v1/partner_force_autonomy/STAGE4_RESULTS_INTERPRETATION_2026-09-20.md",
        ],
    },
    {
        "name": "Peace process",
...
def main():
    ...
    lines = [
        ...
        "",
        f"Current repository baseline: **{py_total} Python test functions across {len(py_paths)} test files** and",
        f"**{rs_total} Rust test cases across {len(rs_nonzero)} Rust source files**.",
        "",
    ...
    for group in GROUPS:
        ...
        if group["py"]:
            ...
            for path in group["py"]:
                lines.append(f"- {md_link(path)} - {py_count(path)} test functions")
        if group["rs"]:
            lines += ["", "Rust:"]
            for path in group["rs"]:
                count = rs_count(path)
                suffix = f"{count} test cases" if count else "implementation evidence; no local test case in this file"
                lines.append(f"- {md_link(path)} - {suffix}")
        lines.append("")
