...
def _load():
    ...
    if event is not None:
        ...
        event.argtypes = event_argtypes
        event.restype = ctypes.c_int
        plan = getattr(library, "pineland_information_event_plan", None)
        if plan is not None:
            plan.argtypes = [event_argtypes[0], *event_argtypes[3:]] + [
                pointer(ctypes.c_uint32), ctypes.c_size_t,
                pointer(ctypes.c_uint32), ctypes.c_size_t,
                pointer(ctypes.c_uint64), ctypes.c_size_t,
                pointer(ctypes.c_uint64), ctypes.c_size_t,
                pointer(ctypes.c_uint32), ctypes.c_size_t,
                pointer(ctypes.c_uint64), pointer(ctypes.c_double), ctypes.c_size_t,
                pointer(ctypes.c_uint64), pointer(ctypes.c_uint32), ctypes.c_size_t,
...
def information_event_plan_enabled() -> bool:
    """Return whether the one-call source-plan event ABI is available."""
    return (
        information_event_enabled()
        and getattr(_load(), "pineland_information_event_plan", None) is not None
    )
    ...
    *,
    ...
    if output_capacity < 1:
        return NativeInformationEventResult(
            ...
            int(next_sequence), len(selected_sources),
        )
    ...
    return NativeInformationEventResult(
        ...
        int(out_next_sequence.value), len(selected_sources),
    )


def native_information_event_plan(
    rng,
    *,
    initial_sources: array,
    member_sources: array,
    locality_group_offsets: array,
    group_source_offsets: array,
    group_sources: array,
    locality_cumulative_offsets: array,
    cumulative_weights: array,
    locality_fallback_offsets: array,
    ...
    """Run one compiled source-selection and information-event pass.

    The selection ranges are the static numeric form of
    :class:`InformationSourcePlan`.  This function is optional for older
    native DLLs; callers fall back to the exact grouped ABI when it is absent.
    """
    library = _load()
    function = (
        getattr(library, "pineland_information_event_plan", None)
        if library is not None else None
    )
    if function is None or not information_event_plan_enabled():
        return None
    if not hasattr(rng, "getstate") or not hasattr(rng, "setstate"):
        return None
    if any(payload.typecode != "I" for payload in (
        initial_sources, member_sources, group_sources, fallback_sources,
    )):
        raise TypeError("native information selection sources must be array('I')")
    if any(payload.typecode != "Q" for payload in (
        locality_group_offsets, group_source_offsets,
        locality_cumulative_offsets, locality_fallback_offsets,
    )):
        raise TypeError("native information selection offsets must be array('Q')")
    if cumulative_weights.typecode != "d":
        raise TypeError("native information cumulative weights must be array('d')")
    source_count = len(source_observer)
    if source_count < 1:
        return None
    source_arrays = (
    ...
    if any(len(payload) != source_count for payload in source_arrays):
        raise ValueError("native information source arrays have inconsistent lengths")
    if source_control.typecode != "d" or len(source_control) != source_count * 7:
        raise ValueError("source_control must contain seven doubles per source")
    target_count = len(target_actor)
    ...
    if target_present.typecode != "B" or target_formation.typecode != "I" or target_alternate_actor.typecode != "I":
        raise TypeError("native information target code arrays have invalid types")
    if any(payload.typecode != "d" for payload in (
        target_personnel, target_detection_probability,
    )):
        raise TypeError("native information target values must be array('d')")
    locality_count = len(locality_group_offsets) - 1
    if locality_count < 0:
        raise ValueError("locality group offsets must not be empty")
    if len(locality_cumulative_offsets) != locality_count + 1:
        raise ValueError("locality cumulative offsets have an invalid length")
    if len(locality_fallback_offsets) != locality_count + 1:
        raise ValueError("locality fallback offsets have an invalid length")
    if len(group_source_offsets) < 1:
        raise ValueError("group source offsets must contain an initial zero")
    if int(locality_group_offsets[0]) != 0 or int(group_source_offsets[0]) != 0:
        raise ValueError("native information selection offsets must start at zero")
    if int(locality_cumulative_offsets[0]) != 0 or int(locality_fallback_offsets[0]) != 0:
        raise ValueError("native information selection offsets must start at zero")
    if int(locality_group_offsets[-1]) > len(group_source_offsets) - 1:
        raise ValueError("locality group offsets exceed group count")
    if int(group_source_offsets[-1]) > len(group_sources):
        raise ValueError("group source offsets exceed source count")
    if int(locality_cumulative_offsets[-1]) > len(cumulative_weights):
        raise ValueError("cumulative offsets exceed weight count")
    if int(locality_fallback_offsets[-1]) > len(fallback_sources):
        raise ValueError("fallback offsets exceed source count")

    # Every source can emit one control row and one row per target. This is a
    # strict upper bound and lets the native plan write into fixed arrays.
    output_capacity = sum(
        1 + int(target_offsets[index + 1]) - int(target_offsets[index])
    ...
    out_count = ctypes.c_size_t()
    out_next_sequence = ctypes.c_uint64()
    out_source_attempts = ctypes.c_size_t()
    status = function(
        _array_pointer(mt_state, ctypes.c_uint32),
        _array_pointer(source_observer, ctypes.c_uint32),
        _array_pointer(source_node, ctypes.c_uint32),
        ...
        _array_pointer(out_physical_control, ctypes.c_double),
        _array_pointer(out_violence, ctypes.c_double),
        _array_pointer(initial_sources, ctypes.c_uint32), len(initial_sources),
        _array_pointer(member_sources, ctypes.c_uint32), len(member_sources),
        _array_pointer(locality_group_offsets, ctypes.c_uint64), locality_count,
        _array_pointer(group_source_offsets, ctypes.c_uint64), len(group_source_offsets) - 1,
        _array_pointer(group_sources, ctypes.c_uint32), len(group_sources),
        _array_pointer(locality_cumulative_offsets, ctypes.c_uint64),
        _array_pointer(cumulative_weights, ctypes.c_double), len(cumulative_weights),
        _array_pointer(locality_fallback_offsets, ctypes.c_uint64),
        _array_pointer(fallback_sources, ctypes.c_uint32), len(fallback_sources),
        ctypes.byref(out_source_attempts),
    )
    if status != 0:
        raise RuntimeError(f"native information event plan failed: {status}")
    count = int(out_count.value)
    if count < 0 or count > output_capacity:
        raise RuntimeError("native information event plan returned an invalid row count")
    rng.setstate((3, tuple(int(value) for value in mt_state), state[2]))
    return NativeInformationEventResult(
        array("I", out_source_index[:count]),
        array("B", out_kind[:count]),
